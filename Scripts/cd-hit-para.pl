...
my $host_no           = 0;
my @hosts             = ();
my $cd_hit_div_exe    = "$script_dir/cd-hit-div-big";
...
while ($arg=shift) {
    ...
    elsif ($arg eq "--q") { $qname      = shift; }
    elsif ($arg eq "--p") { $pname      = shift; }
    elsif ($arg eq "--t") { $thread     = shift; }
    elsif ($arg eq "--m") { $mem        = shift; }
...
$pname = "-P $pname" if $pname;
$qname = "-q $qname" if $qname;
...
my $cmd;
my ($i, $j, $k, $i1, $j1, $k1);
my $lock_dir       = "$work_dir/$out-lock";
my $qsub_log       = "$work_dir/$out-qsub_log";
...
EOD
        ...
        if ($local_cpu) {
            ...
            my $t = "para-$thost_idx";
            open(QUEUE,"| qsub -cwd -N $t -e $qsub_log/$i.log -o $qsub_log/$i.log -l vf=$mem,p=$thread -binding linear:$thread $qname $pname");
            print QUEUE <<EOD;
cd $pwd
sh $tsh
...
sub write_restart {
    my ($i, $j, $k);
    open RES, ">$restart_file.temp"; 
    for ($i=0; $i<$command_no; $i++) {
        print RES "$commands[$i]\n$command_status[$i]\n";
    }
    close RES; 
    system(`mv -f $restart_file.temp $restart_file`) || die($!);
}
########## END write_restart
...
Options 
    ...
    --q queue name for qsub -q (default none)
    --p project name for qsub -P (default none)
    --t number of threads for qsub -l p=
    --m memory for qsub -l vf=
     -h print this help
